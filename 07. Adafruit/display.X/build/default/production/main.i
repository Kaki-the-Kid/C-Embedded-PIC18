...
void adafruit_init() {
    i2c_WriteSerial(display_Addr, procent_sign, 11);
    i2c_WriteSerial(display_Addr, degree_sign, 11);
}

void adafruit() {
    i2c_WriteSerial(display_Addr, clearDisplay, 2);


    i2c_WriteSerial(adafruit_Addr, adafruit_humi_command, 1);
    i2c_ReadSerial(adafruit_Addr, humiRead, 2);

    float rh_code = ((humiRead[0] << 8) + humiRead[1]);
    float rh = ((125 * rh_code) / 65536) - 6;



    displayLine[1] = 0x80;
    i2c_WriteSerial(display_Addr, displayLine, 2);
    i2c_WriteSerial(display_Addr, humiText, 6);
    ...
    i2c_WriteSerial(display_Addr, outH, sizeof(outH));

    costum_char[1] = 0x01;
    i2c_WriteSerial(display_Addr, costum_char, 2);



    i2c_WriteSerial(adafruit_Addr, adafruit_temp_command, 1);
    i2c_ReadSerial(adafruit_Addr, tempRead, 2);

    float temp = ((175.72 * ((tempRead[0] << 8) + tempRead[1])) / 65536) - 46.85;



    displayLine[1] = 0xC0;
    i2c_WriteSerial(display_Addr, displayLine, 2);
    i2c_WriteSerial(display_Addr, tempText, 6);
    ...
    i2c_WriteSerial(display_Addr, outT, sizeof(outT));
...
void dataLogger_init() {
    ...
    i2c_WriteSerial(eeprom_Addr, verifyPos, 2);
    _delay((unsigned long)((10)*(64000000/4000.0)));

    i2c_ReadSerial(eeprom_Addr, logStatus, 1);
    _delay((unsigned long)((10)*(64000000/4000.0)));
    ...
    i2c_WriteSerial(eeprom_Addr, nextPos, 2);
    _delay((unsigned long)((10)*(64000000/4000.0)));

    i2c_ReadSerial(eeprom_Addr, logPos, 2);
    _delay((unsigned long)((10)*(64000000/4000.0)));
    ...
    if((logPos[0] == 0x00) && (logPos[1] < 0x03)) {
        ...
        i2c_WriteSerial(eeprom_Addr, nextPos, 4);
        _delay((unsigned long)((10)*(64000000/4000.0)));
...
# 498 "main.c"
    if ((logStatus[0] != 0x02) && (logStatus[0] != 0x01)){
        ...
        i2c_WriteSerial(eeprom_Addr, verifyPos, 3);
        _delay((unsigned long)((10)*(64000000/4000.0)));
...
void dataLogWrite() {
    ...
    logData[0] = verifyPos[0];
    logData[1] = verifyPos[1];
    logData[2] = logStatus[1];

    i2c_WriteSerial(eeprom_Addr, logData, 3);
    _delay((unsigned long)((10)*(64000000/4000.0)));
    ...
    logData[0] = logPos[0];
    logData[1] = logPos[1];
    logData[2] = tempRead[0];
    logData[3] = tempRead[1];

    i2c_WriteSerial(eeprom_Addr, logData, 4);
    _delay((unsigned long)((10)*(64000000/4000.0)));
    ...
    if(logStatus[0] == 0x01) { logStatus[0] = 0x02; }
    else { logStatus[0] = 0x01; }

    logData[2] = logStatus[0];
...
void dataLogRead() {
    i2c_WriteSerial(eeprom_Addr, readPos, 2);
    _delay((unsigned long)((10)*(64000000/4000.0)));

    if ((readPos[1] + 2) != 0xFF) { readPos[1] += 4; }
    ...
    i2c_ReadSerial(eeprom_Addr, readData, 4);
    _delay((unsigned long)((10)*(64000000/4000.0)));

    float readTemp;
    char readOutT[5];

    readTemp = ((175.72 * ((readData[0] << 8) + readData[1])) / 65536) - 46.85;

    sprintf(readOutT, "%.1f", readTemp);
    printf("1st Read data: %s \r\n", readOutT);
    _delay((unsigned long)((10)*(64000000/4000.0)));

    readTemp = ((175.72 * ((readData[2] << 8) + readData[3])) / 65536) - 46.85;
