...
void DisplaySetCursor(_Bool, uint8_t);
float ReadHumidity();
...
void main(void) {
    ...
    I2cWriteSerial(displayAddress, displayInitialize, 5);
    _delay((unsigned long)((10)*(64000000/4000.0)));

    I2cWriteSerial(displayAddress, customSign0, 11);
    I2cWriteSerial(displayAddress, customSign1, 11);
...
void Timer4Interrupt() {
    ...
    if (counter == 100) {
        ...
        uint8_t output[4];
        _delay((unsigned long)((5)*(64000000/4000.0)));
        I2cReadSerial(eepromAddress, output, 2);
...
void I2c_init() {
    ...
    uint8_t i2cClock = (64000000 / (4 * 100000) - 1);
...
void I2cWriteSerial(uint8_t address, uint8_t * writeData, uint8_t length) {
    ...
    uint8_t addressWrite = address << 1;

    SSP1CON2bits.SEN = 1;
    while (SSP1CON2bits.SEN == 1);

    SSP1BUF = addressWrite;
...
void DisplaySetCursor(_Bool line, uint8_t charNum) {
    uint8_t command[2];
    command[0] = 0x00;

    if (line == 1) {
        command[1] = 0xC0 + charNum;
    } else {
        command[1] = 0x80 + charNum;
    }
    I2cWriteSerial(displayAddress, command, 2);
}
...
void WriteTempHumidToDisplay() {
    ...
    I2cWriteSerial(displayAddress, "@Temp: ", 7);
    DisplaySetCursor(1, 0);
    I2cWriteSerial(displayAddress, "@Humidity:", 10);
    ...
    sprintf(printable, "@%.1f", result);

    printable[5] = 0x00;
    I2cWriteSerial(displayAddress, printable, 6);
    I2cWriteSerial(displayAddress, "@  ", 2);
...
void LogData() {
    ...
    _delay((unsigned long)((5)*(64000000/4000.0)));
    I2cWriteSerial(eepromAddress, writeArray, 6);
