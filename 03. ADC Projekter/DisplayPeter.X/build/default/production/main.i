...
uint8_t updateTempratureCounter = 100;
uint8_t numberSystem = 10;

_Bool displayChoice = 0;
    ...
    while (1)
    {
        ...
        if(EUSART1_is_tx_ready())
        {
            while(1)
            {
                ...
                char actionSelector[12];
                strcpy(actionSelector, ReadLine());

                if(!strcmp(actionSelector, ("1\r")))
                {
                    printf("The temperature is %d degrees Celsius", temperature);
                }
                else if(!strcmp(actionSelector, ("2\r")))
                {
                    numberSystem= 16;
                }
                else if(!strcmp(actionSelector, ("3\r")))
                {
                    numberSystem= 10;
                }
                else
...
void UpdateDisplay()
{
    ...
    if(displayChoice)
    {
        uint8_t firstDigit = temperature / numberSystem;
        SetDisplay(firstDigit,1);
        displayChoice = 0;
    }
    else
    {
        uint8_t secondDigit = temperature % numberSystem;
        SetDisplay(secondDigit,0);
        displayChoice = 1;
    }
}


void SetDisplay(uint8_t digit, _Bool firstDigit)
{
    if(firstDigit)
    {
...
void UpdateTemprature()
{
    ...
    {
        sum += ADC_GetConversion(TempSensor);
    }
    ...
    uint16_t temp_offset = average - 500;

    roundingTest = temp_offset % 10;
    temperature = temp_offset /10;
    if (roundingTest >= 5)
    {
        temperature++;
    }
...
const char * ReadLine()
{
    ...
    while(1)
    {
        ...
        {
            int tmp = EUSART1_Read();

            printf("%c", (unsigned char)tmp);
            line[index] = tmp;
            index++;
            line[index] = 0x00;
            if(index > 11)
